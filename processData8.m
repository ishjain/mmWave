...
for jj = 1:size(res,1) %3 for freq, dur, pB and 3 for their intrvl
    ...
    res3{jj} = reshape(res2{jj}(1:end-1,:,[1,2,3]),nAP-1,3*3);
...
colTitle= {'lamT','lamB0.01omega0','lamB0.1omega0','lamB0.2omega0',...
    ...
    'lamB0.01omega0int','lamB0.1omega0int','lamB0.2omega0int',...
    'lamB0.01omega60int','lamB0.1omega60int','lamB0.2omega60int'};
...
colTitle= {'lamT','lamB0.01omega0','lamB0.1omega0','lamB0.2omega0',...
    ...
    'lamB0.01omega0int','lamB0.1omega0int','lamB0.2omega0int',...
    'lamB0.01omega60int','lamB0.1omega60int','lamB0.2omega60int',...
    'lamB0.01omega90int','lamB0.1omega90int','lamB0.2omega90int'};
...
% %     ylim([1e-4,1])
    figure(4);
    semilogy(densityAP,freqCond(:,1:6));
    title('Conditional expectation of freq of bl given n!=0')
%     ylim([1e-4,1])
    legend(legendArray);
    
    figure(5);
    semilogy(densityAP,durCond(:,1:6))

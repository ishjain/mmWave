%Theory_LOS: June6: Renamed Theory2 to Theory_LOS
...
% !!!-------------------------------!!!
pB = zeros(length(densityAP),length(densityBL),length(omegaVal));
pBCond=zeros(length(densityAP),length(densityBL),length(omegaVal));
freq=zeros(length(densityAP),length(densityBL),length(omegaVal));
freqCond=zeros(length(densityAP),length(densityBL),length(omegaVal));
durCond=zeros(length(densityAP),length(densityBL),length(omegaVal));
...
for indT = 1:length(densityAP)
    for indB = 1:length(densityBL)
        for indO = 1:length(omegaVal)
            ...
            a = 2*mu/(R*C(indB))-2*mu^2/(R^2*C(indB)^2)*log(1+R*C(indB)/mu);
            pnn0(indT,indO)=exp(-p*lamT*pi*R^2);
            
        
       
            %1. Marginal prob of Blockage
            pB(indT,indB,indO) = exp(-a*p*lamT*pi*R^2);
            ...
            freq(indT,indB,indO) = mu*(1-a)*p*lamT*pi*R^2*exp(-a*p*lamT*pi*R^2);
            ...
            %This is used to get
            b(indT,indO) = p*lamT*pi*R^2;%*angFrac;
%             ab(indT,indB,indO) = b(indT,indO)*a_tilde(indB,indO);%*b(indT);
            ...
            %approx
            durCond3(indT,indB,indO) = (1/b(indT,indO)+1/b(indT,indO)^2)/...
                (mu*(1-pnn0(indT,indO)));
...
%save csv file
%%Make it 2D arrays
pB=reshape(pB(:,:,[1,2,3]),length(densityAP),3*3);
% pB_tilde=reshape(pB_tilde(:,:,[1,2,3]),length(densityAP),3*3);
pBCond=reshape(pBCond(:,:,[1,2,3]),length(densityAP),3*3);
freq=reshape(freq(:,:,[1,2,3]),length(densityAP),3*3);
freqCond=reshape(freqCond(:,:,[1,2,3]),length(densityAP),3*3);
durCond=reshape(durCond(:,:,[1,2,3]),length(densityAP),3*3);
durCond2=reshape(durCond2(:,:,[1,2,3]),length(densityAP),3*3);
durCond3=reshape(durCond3(:,:,[1,2,3]),length(densityAP),3*3);
